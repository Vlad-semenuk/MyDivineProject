...
public class UnblockUserCommand extends Command {
    ...
    /**
     * Unblocked user by login by administrator
     * @param req
     * @param resp
    ...
    public String execute(HttpServletRequest req, HttpServletResponse resp) throws IOException, AppException {
        ...
        LOG.trace(String.format("Unblock user by login %s", login));

        User user = FACADE.getUserByLogin(login);
        LOGIN_VALIDATOR.validate(user.getLogin());

        FACADE.setUserFieldBlockedByUserLogin(false, user.getLogin());
